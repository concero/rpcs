...
function isValidRpcsResponse(chainIdHex: unknown) {
...
function validateChainId(chainIdHex: unknown): string | null {
    if (!isValidRpcsResponse(chainIdHex)) return null;
...
async function testSingleEndpoint(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            const returnedChainId = validateChainId(responseBody[0].result);
            if (!returnedChainId) {
                debug(`Invalid chainId from ${url}: ${responseBody[0]}`);
                return null;
            }

            const finalityTagEnabled = checkFinalityTagEnabled(responseBody[1]?.result);
...
function checkFinalityTagEnabled(rpcsResponse: unknown) {
    return Boolean((rpcsResponse as any).number);
}
    ...
    return new Promise<RpcTestResult>((resolve, reject) => {
        function processNext(): void {
            ...
            if (queue.length === 0 && activeCount === 0) {
                info(
                    `Testing complete: ${healthyRpcs.length}/${endpoints.length} endpoints healthy`,
                );
                    ...
                    .finally(() => {
                        ...
                        if (processedCount % progressThreshold === 0) {
                            const percentage = Math.round(
                                (processedCount / endpoints.length) * 100,
                            );
                            info(
                                `Progress: ${percentage}% (${processedCount}/${endpoints.length})`,
                            );
                        }
